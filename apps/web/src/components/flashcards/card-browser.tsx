        ...
        <TabsList className="bg-zinc-900/80 border border-zinc-800 h-auto flex-wrap">
          {DOMAIN_TABS.map((tab) => (
            <TabsTrigger
              ...
              value={tab.slug}
              className={cn(
                "text-xs data-[state=active]:bg-blue-500/10 data-[state=active]:text-blue-400 data-[state=active]:border-blue-500/20",
                "text-zinc-500 hover:text-zinc-300"
              )}
            >
              {tab.label}
              {(dueCounts[tab.slug] ?? 0) > 0 && (
                <span className="ml-1.5 inline-flex items-center justify-center h-4 min-w-[16px] px-1 rounded-full bg-blue-500/20 text-blue-400 text-[9px] font-bold">
                  {dueCounts[tab.slug]}
                </span>
              )}
            </TabsTrigger>
                          ...
                          <div className="mt-4 pt-4 border-t border-zinc-800 space-y-3">
                            ...
                            {card.sourceUrl && (
                              <a
                                ...
                                className="inline-flex items-center gap-1 text-xs text-blue-500/70 hover:text-blue-400 transition-colors"
