...
function renderMarkdown(text: string): string {
  ...
  let html = escapeHtml(text)
    ...
    .replace(
      /```(?:\w*)\n([\s\S]*?)```/g,
      '<pre class="bg-zinc-950 border border-zinc-800 rounded-lg p-4 overflow-x-auto my-4"><code class="text-sm text-blue-400 font-mono">$1</code></pre>'
    )
    // Inline code (backtick)
    .replace(
      /`([^`]+)`/g,
      '<code class="bg-zinc-800 text-blue-400 px-1.5 py-0.5 rounded text-sm font-mono">$1</code>'
    )
    // Headers
    .replace(
      /^### (.+)$/gm,
          ...
          <Tabs defaultValue="instructions" className="flex flex-col flex-1 min-h-0 gap-0">
            <div className="shrink-0 border-b border-zinc-800 px-4">
              <TabsList className="bg-transparent h-10 gap-0 p-0 rounded-none">
                <TabsTrigger
                  value="instructions"
                  className="data-[state=active]:bg-transparent data-[state=active]:text-blue-400 data-[state=active]:border-b-2 data-[state=active]:border-blue-400 rounded-none text-xs text-zinc-500 px-4 h-10"
                >
