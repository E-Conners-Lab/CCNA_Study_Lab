# CCNA StudyLab -- 8-Phase Refactor
...
- **Domain data was hardcoded in 5+ files.** Every page that mentioned the six CCNA exam domains had its own copy of the slug, name, weight, and number arrays. Updating the exam blueprint would have required hunting through unrelated files.
...
**Problem:** The six CCNA 200-301 exam domains (slugs, names, weights, numbers) were hardcoded in at least five places:
...
The tutor sidebar now imports `CCNA_DOMAINS` and `getDomainBySlug` from `@/lib/domains` instead of carrying its own domain array.
...
- Replaced the ~90-line `defaultDomains` array in `dashboard/page.tsx` with a derived array from `CCNA_DOMAINS`.
...
- `src/app/dashboard/page.tsx` -- reduced by ~80 lines, now derives defaults from `CCNA_DOMAINS`
...
|--------|------|-------------|
| `CCNADomain` | interface | Shape: `{ number, slug, name, shortName, weight }` |
| `CCNA_DOMAINS` | `readonly CCNADomain[]` | The six exam domains |
...
CCNA_DOMAINS.map(d => <Tab key={d.slug}>D{d.number} {d.shortName}</Tab>);
```
