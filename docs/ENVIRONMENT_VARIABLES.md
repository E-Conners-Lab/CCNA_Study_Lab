# CCNA StudyLab -- Environment Variables
...
When unset, labs that support local execution (Python) will run code via a local `python3` subprocess. Labs requiring the engine (IOS CLI simulation, subnetting, config review, ACL builder) will display a message that the lab engine is required.
