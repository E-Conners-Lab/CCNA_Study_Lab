...
```bash
git clone https://github.com/E-Conners-Lab/CCNA_StudyLab.git ccna-studylab
cd ccna-studylab
...
Default connection: `postgresql://studylab:studylab_dev_2024@localhost:5433/ccna_studylab`
...
# Also creates the default dev user: student@ccna.lab / ccna123
npm run db:seed
```
...
The hands-on labs require the lab engine. Start all Docker services:
...
| Lab Engine | 8100 | Code execution and grading (IOS CLI, subnetting, ACL, config review) |
...
Another PostgreSQL instance is running on port 5433. Either stop it or change the port in `docker-compose.yml` and update `DATABASE_URL` in `.env.local` to match.
